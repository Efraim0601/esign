...
RSpec.describe Users::BulkImport do
  ...
  describe '.parse_xlsx' do
    ...
    def build_cell(value)
      double('cell', value: value)
    end

    def build_row(cells)
      double('xrow', cells: cells)
    end

    it 'parses xlsx rows including header row and skips blank rows' do
      header_row = build_row([build_cell('email'), build_cell('first_name'),
                              build_cell('last_name'), build_cell('role')])
      data_row = build_row([build_cell('user@example.test'), build_cell('User'),
                            build_cell('One'), build_cell('admin')])
      blank_row = build_row([build_cell(nil), build_cell(''), build_cell(''), build_cell('')])

      allow(workbook).to receive(:worksheets).and_return([worksheet])
      allow(worksheet).to receive(:each_with_index)
        .and_yield(header_row, 0).and_yield(data_row, 1).and_yield(blank_row, 2).and_yield(nil, 3)
      allow(RubyXL::Parser).to receive(:parse_buffer).and_return(workbook)

      file = double('file', read: 'xlsx-bytes')

      rows = described_class.parse_xlsx(file)

      expect(rows.size).to eq(1)
      expect(rows.first.email).to eq('user@example.test')
      expect(rows.first.first_name).to eq('User')
      expect(rows.first.last_name).to eq('One')
      expect(rows.first.role).to eq('admin')
    end

    it 'raises InvalidFile when the workbook has no worksheets' do
      allow(workbook).to receive(:worksheets).and_return([nil])
      allow(RubyXL::Parser).to receive(:parse_buffer).and_return(workbook)
      file = double('file', read: 'xlsx-bytes')

      expect do
        described_class.parse_xlsx(file)
      end.to raise_error(Users::BulkImport::InvalidFile, /Fichier XLSX vide/)
    end

    it 'raises InvalidFile when headers are missing' do
      header_row = build_row([build_cell('only_email')])

      allow(workbook).to receive(:worksheets).and_return([worksheet])
      allow(worksheet).to receive(:each_with_index).and_yield(header_row, 0)
      allow(RubyXL::Parser).to receive(:parse_buffer).and_return(workbook)
      file = double('file', read: 'xlsx-bytes')

      expect do
        described_class.parse_xlsx(file)
      end.to raise_error(Users::BulkImport::InvalidFile, /Colonnes manquantes|XLSX illisible/)
    end

    it 'wraps any raised error in InvalidFile' do
      allow(RubyXL::Parser).to receive(:parse_buffer).and_raise(StandardError.new('mangled'))
      file = double('file', read: 'broken')

      expect do
        described_class.parse_xlsx(file)
      end.to raise_error(Users::BulkImport::InvalidFile, /XLSX illisible: mangled/)
    end
  end

  describe '.parse_csv extension fallback' do
    it 'parses csv when filename has no extension' do
      file = double('file', original_filename: 'users', read: "email,first_name,last_name,role\na@x.test,A,B,member\n")
      allow(file).to receive(:respond_to?).with(:original_filename).and_return(true)

      rows = described_class.parse_rows(file)

      expect(rows.size).to eq(1)
      expect(rows.first.email).to eq('a@x.test')
    end

    it 'detects semicolon separator' do
      file = double('file', read: "email;first_name;last_name;role\nb@x.test;B;C;admin\n")

      rows = described_class.parse_csv(file)

      expect(rows.first.email).to eq('b@x.test')
      expect(rows.first.role).to eq('admin')
    end

    it 'skips rows where every value is blank' do
      file = double('file', read: "email,first_name,last_name,role\n,,,\nc@x.test,C,D,admin\n")

      rows = described_class.parse_csv(file)

      expect(rows.size).to eq(1)
      expect(rows.first.email).to eq('c@x.test')
    end
