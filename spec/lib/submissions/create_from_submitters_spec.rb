...
RSpec.describe Submissions::CreateFromSubmitters do
  ...
  describe '.maybe_set_dynamic_documents' do
    it 'returns submission unchanged when no template is associated' do
      submission = double('submission', template_id?: false)

      expect(described_class.maybe_set_dynamic_documents(submission)).to eq(submission)
    end

    it 'returns submission unchanged when variables_schema is present on submission' do
      submission = double('submission', template_id?: true, variables_schema: ['x'])
      allow(submission).to receive(:template).and_return(double(variables_schema: nil))

      expect(described_class.maybe_set_dynamic_documents(submission)).to eq(submission)
    end

    it 'returns submission unchanged when variables_schema is present on template' do
      template = double('template', variables_schema: ['t'])
      submission = double('submission', template_id?: true, variables_schema: nil, template: template)

      expect(described_class.maybe_set_dynamic_documents(submission)).to eq(submission)
    end

    it 'builds template_schema with deep-dup items for non-dynamic schema items' do
      schema = [{ 'attachment_uuid' => 'doc-1', 'dynamic' => false }]
      template = double('template', variables_schema: nil, schema: schema, fields: [])
      submission_schema_store = []
      submission_fields_store = []
      submission = double('submission', template_id?: true, variables_schema: nil, template: template)
      allow(submission).to receive(:template_schema=) { |v| submission_schema_store = v }
      allow(submission).to receive(:template_schema) { submission_schema_store }
      allow(submission).to receive(:template_fields=) { |v| submission_fields_store = v }
      allow(submission).to receive(:template_fields) { submission_fields_store }

      described_class.maybe_set_dynamic_documents(submission)

      expect(submission_schema_store.first['attachment_uuid']).to eq('doc-1')
      expect(submission_schema_store.first).not_to have_key('dynamic_document_sha1')
    end
  end

  describe '.maybe_add_invite_submitters' do
    it 'skips submitters already in template_submitters list' do
      submission = double('submission')
      template_submitters_store = [{ 'uuid' => 'u1', 'invite_by_uuid' => 'parent' }]
      allow(submission).to receive(:template_submitters).and_return(template_submitters_store)
      template = double('template', submitters: [{ 'uuid' => 'u1', 'name' => 'Manager', 'invite_by_uuid' => 'parent' }])

      described_class.maybe_add_invite_submitters(submission, template, [])

      expect(template_submitters_store.size).to eq(1)
    end

    it 'inserts invite-by-uuid submitter at index 1 for first position' do
      submission = double('submission')
      template_submitters_store = [{ 'uuid' => 'main' }]
      allow(submission).to receive(:template_submitters).and_return(template_submitters_store)
      template = double('template', submitters: [{ 'uuid' => 'u1', 'name' => 'Signer', 'invite_by_uuid' => 'main' }])

      described_class.maybe_add_invite_submitters(submission, template, [])

      expect(template_submitters_store.last['uuid']).to eq('u1')
    end

    it 'appends submitter when index exceeds current submitters size' do
      submission = double('submission')
      template_submitters_store = [{ 'uuid' => 'first' }]
      allow(submission).to receive(:template_submitters).and_return(template_submitters_store)
      template = double('template', submitters: [
                          { 'uuid' => 'first', 'name' => 'A' },
                          { 'uuid' => 'second', 'name' => 'B', 'optional_invite_by_uuid' => 'first' }
                        ])

      described_class.maybe_add_invite_submitters(submission, template, [])

      expect(template_submitters_store.map { |s| s['uuid'] }).to eq(%w[first second])
    end

    it 'resolves invite_by from submitter attributes' do
      submission = double('submission')
      template_submitters_store = [{ 'uuid' => 'first' }]
      allow(submission).to receive(:template_submitters).and_return(template_submitters_store)
      template = double('template', submitters: [
                          { 'uuid' => 'first', 'name' => 'Manager' },
                          { 'uuid' => 'second', 'name' => 'Signer' }
                        ])

      described_class.maybe_add_invite_submitters(submission, template,
                                                  [{ 'role' => 'Signer', 'invite_by' => 'Manager' }])

      expect(template_submitters_store.map { |s| s['uuid'] }).to include('second')
      expect(template_submitters_store.find { |s| s['uuid'] == 'second' }['invite_by_uuid']).to eq('first')
    end
  end
end
