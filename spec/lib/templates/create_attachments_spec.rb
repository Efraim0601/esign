...
RSpec.describe Templates::CreateAttachments do
  describe '.extract_zip_files' do
    ...
    it 'extracts supported files from zip entries' do
      tempfile = Tempfile.new(['upload', '.zip'])
      file = double('file', content_type: Templates::CreateAttachments::ZIP_CONTENT_TYPE, tempfile: tempfile)
      entry = double('entry', directory?: false, size: 100, name: 'doc.pdf')
      stream = StringIO.new('pdf-bytes')
      zip_file = double('zip_file')
      allow(Zip::File).to receive(:open).with(tempfile).and_return(zip_file)
      allow(zip_file).to receive(:each).and_yield(entry)
      allow(entry).to receive(:get_input_stream).and_yield(stream)
      allow(Marcel::MimeType).to receive(:for).and_return('application/pdf')

      files = described_class.extract_zip_files(file)
