...
describe 'UsersController' do
  ...
  describe 'PATCH /users/:id' do
    ...
    it 'records role change, increments permission_version and forgets remember token' do
      target = create(:user, account:, role: 'member', remember_created_at: Time.current)
      allow(UserMailer).to receive(:role_changed).and_return(double(deliver_later: true))

      expect do
        patch "/users/#{target.id}", params: { user: { role: 'editor' } }
      end.to change { RoleChangeLog.count }.by(1)

      target.reload
      expect(target.role).to eq('editor')
      expect(target.permission_version).to be >= 1
      expect(target.remember_created_at).to be_nil
      expect(UserMailer).to have_received(:role_changed).with(target, 'member', 'editor', admin.id)
    end

    it 'still finishes the request when role_changed mailer fails to enqueue' do
      target = create(:user, account:, role: 'member')
      mailer = double('mailer')
      allow(UserMailer).to receive(:role_changed).and_return(mailer)
      allow(mailer).to receive(:deliver_later).and_raise(StandardError.new('queue down'))
      allow(Rails.logger).to receive(:error)

      patch "/users/#{target.id}", params: { user: { role: 'editor' } }

      expect(response).to be_redirect
      expect(Rails.logger).to have_received(:error).with(/UserRoleChange/)
    end
  end
