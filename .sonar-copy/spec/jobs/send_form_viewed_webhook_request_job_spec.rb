...
RSpec.describe SendFormViewedWebhookRequestJob do
  ...
  describe '#perform' do
    ...
    it 'returns without sending when webhook url has no matching event' do
      webhook_url.update!(events: ['submission.created'])

      described_class.new.perform('submitter_id' => submitter.id, 'webhook_url_id' => webhook_url.id, 'event_uuid' => SecureRandom.uuid)
