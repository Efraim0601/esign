...
describe 'Form Events API' do
  ...
  describe 'GET /api/events/form/:type' do
    it 'returns 401 when no auth token is provided' do
      get '/api/events/form/completed'

      expect(response).to have_http_status(:unauthorized)
    end

    it 'returns an empty list when no submitters are completed' do
      create(:submission, :with_submitters, template:, created_by_user: author)

      get '/api/events/form/completed', headers: { 'x-auth-token': author.access_token.token }
    ...
    it 'returns completed submitter events' do
      ...
      expect(data.first['event_type']).to eq('form.completed')
